...
Local Deep Researcher is a **significantly modified version** of the original `langchain-ai/local-deep-researcher`, enhanced with new features including embedded corpora and advanced research loop logic. It uses any LLM hosted by [Ollama](https://ollama.com/search) or [LMStudio](https://lmstudio.ai/). **Note:** This version is currently intended for specific academic and educational use under a custom license (see License section below). Give it a topic and it will generate a web search query, gather web search results, summarize the results of web search, reflect on the summary to examine knowledge gaps, generate a new search query to address the gaps, and repeat for a user-defined number of cycles. It will provide the user a final markdown summary with all sources used to generate the summary.

![ollama-deep-research](https://github.com/user-attachments/assets/1c6b28f8-6b64-42ba-a491-1ab2875d50ea)

<!-- Original README content below, adapted from langchain-ai/local-deep-researcher -->
...
2. In LMStudio:
   - Download and load your preferred model (e.g., deepseek-r1:14b)
...
LOCAL_LLM=deepseek-r1:14b  # Use the exact model name as shown in LMStudio
...
Local Deep Researcher works as follows:

### Single-Path Research (Original Mode)

This approach is inspired by [IterDRAG](https://arxiv.org/html/2410.04343v1#:~:text=To%20tackle%20this%20issue%2C%20we,used%20to%20generate%20intermediate%20answers.).
- Given a user-provided topic, use a local LLM to generate a web search query
...
This project is based on `langchain-ai/local-deep-researcher`, which is licensed under the MIT License.

However, the significant modifications and additions in this version are provided under a **custom license** detailed in the `LICENSE` file. Please review it carefully.

**Key Terms:**
- **Permitted Use:** Non-commercial, academic research and educational purposes ONLY. Specifically intended for use during the [Hackathon Name/Date] hackathon and for internal research by [Rui Ding/Junhong Chen's Lab].
- **Restrictions:**
    - **NO** commercial use.
    - **NO** redistribution or creation of derivative works without express written permission from [Rui Ding/Junhong Chen's Lab].
    - **NO** disclosure of proprietary components outside permitted use.
- **Citation Required:** Any publications or presentations using this tool MUST cite the work. Contact [ruiding@uchciago.edu] for details.
